...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ========================================================
  // 🎬 Auto-Pilot Demo Recording Suite (Screen Studio 自动演示)
  // ========================================================
  let demoAbort = false;

  const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

  async function moveCursor(toX, toY, durationMs = 800) {
    if (demoAbort) return;
    const startX = virtualCursor.dataset.x ? parseFloat(virtualCursor.dataset.x) : window.innerWidth / 2;
    const startY = virtualCursor.dataset.y ? parseFloat(virtualCursor.dataset.y) : window.innerHeight / 2;
    ...
    return new Promise((resolve) => {
      function step(now) {
        ...
        virtualCursor.dataset.x = curX;
        virtualCursor.dataset.y = curY;

        handlePointerMove({ clientX: curX, clientY: curY });
  ...
  async function clickVirtual(element, moveTime = 600) {
    if (demoAbort || !element) return;
    const rect = element.getBoundingClientRect();
    const targetX = rect.left + rect.width / 2;
    const targetY = rect.top + rect.height / 2;

    await moveCursor(targetX, targetY, moveTime);
    if (demoAbort) return;
  ...
  async function dragSliderVirtual(slider, valSpan, targetValue, unit, stateProp, duration = 800) {
    if (demoAbort || !slider) return;
    const rect = slider.getBoundingClientRect();
    await moveCursor(rect.left + 20, rect.top + rect.height / 2, 500);
    if (demoAbort) return;
    ...
    return new Promise((resolve) => {
      function step(now) {
        ...
        const ease = progress < 0.5 ? 4 * progress * progress * progress : 1 - Math.pow(-2 * progress + 2, 3) / 2;
        const currentVal = Math.round(startVal + (targetValue - startVal) * ease);

        slider.value = currentVal;
        if (valSpan) valSpan.textContent = `${currentVal}${unit}`;
        state[stateProp] = currentVal;
        applyState();

        const sliderX = rect.left + (rect.width * progress);
        virtualCursor.style.left = `${sliderX}px`;
        virtualCursor.dataset.x = sliderX;
  ...
  async function runAutoDemo() {
    if (isDemoRunning) {
      ...
      btnAutoDemo.textContent = '✨ 自动演示录屏';
      btnAutoDemo.classList.remove('btn-primary');
      btnAutoDemo.classList.add('btn-demo');
      return;
    }
    ...
    try {
      ...
      // 2. [4-8s] 暗角与圆角微调
      await dragSliderVirtual(sliderVignette, valVignette, 65, '%', 'vignette', 900);
      await sleep(300);
      await dragSliderVirtual(sliderRadius, valRadius, 12, 'px', 'radius', 700);
      await sleep(400);

      // 3. [8-12s] 相纸底色与调色板拾色
      const pinkBtn = document.querySelector('.paper-btn[data-paper="pink"]');
      if (pinkBtn) await clickVirtual(pinkBtn, 600);
      await sleep(600);

      const blackBtn = document.querySelector('.paper-btn[data-paper="black"]');
      if (blackBtn) await clickVirtual(blackBtn, 500);
      await sleep(800);

      const goldInk = document.querySelector('.ink-dot[data-ink="#d97706"]');
      if (goldInk) await clickVirtual(goldInk, 500);
      await sleep(800);

      // 4. [12-16s] 切换至双人情侣对卡模式
      const dualTab = document.getElementById('tabDual');
      if (dualTab) await clickVirtual(dualTab, 600);
      await sleep(800);

      // 晃动展示双卡浪漫倾角
      updateBounds();
      const dualCenterX = bounds.left + bounds.width / 2;
      const dualCenterY = bounds.top + bounds.height / 2;
      await moveCursor(dualCenterX - 100, dualCenterY, 600);
      await sleep(300);
      await moveCursor(dualCenterX + 100, dualCenterY, 800);
      await sleep(500);

      // 5. [16-20s] 开启 360° 悬浮自转
      const spinBtn = document.getElementById('btnAutoSpin');
      if (spinBtn) await clickVirtual(spinBtn, 600);
      await sleep(3500); // 欣赏优雅自转

      // 6. [20-23s] 点击导出
      const exportBtn = document.getElementById('btnExport');
      if (exportBtn) await clickVirtual(exportBtn, 600);
      await sleep(2000);
